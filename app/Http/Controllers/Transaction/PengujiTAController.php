...
class PengujiTAController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $akademik = PengujiTugasAkhir::whereHas('sgas', function (Builder $query) use ($request) {
                $query->where('id_dosen', $request->dosen)
                    ->where('id_tahun_akademik', $request->ta)
                    ->where('jenis_kegiatan', PengujiTugasAkhir::class)
                    ->where('semester', $request->semester);
            })->get();
            return ResponseFormatter::success($akademik, 'Data berhasil diambil!');
        }

        return view('pages.transaction.penguji_ta', compact('dosen', 'ta', 'matakuliah'));
    }
        ...
        try {
            ...
            if ($ceksgas == null || $ceksgas == '') {
                Sgas::create([
                    ...
                    'jenis_kegiatan' => PengujiTugasAkhir::class,
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $pengajaran = PengujiTugasAkhir::create([
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $pengajaran = PengujiTugasAkhir::where('id', $request->pa)->update([
                'nama_mahasiswa' => $request->nama_mahasiswa,
                'nim' => $request->nim,
                'peran' => $request->peran,
                'judul_ta' => $request->judul_ta
    ...
    {
        try {
            $pengajaran = PengujiTugasAkhir::where('id', $request->id)->delete();
    ...
    public function print_all(Request $request)
    {
        ...
        $pengajaran = PengujiTugasAkhir::whereHas('sgas',  function (Builder $query) use ($id) {
            $query->where('id', $id);
        })->get();
        ...
        foreach ($pengajaran as $k => $v) {
            ...
            $pdf->Cell(1, 7, ':', 0, 0, 'L');
            $pdf->Cell(5);
            $pdf->MultiCellIndent(147, 7, 'Bahwa dalam rangka melaksanakan aktivitas Tridharma Perguruan Tinggi yaitu penguji Tugas Akhir/Skripsi bagi mahasiswa di ITSK RS dr. Soepraoen, maka perlu di keluarkan surat tugas.', 0, 'J', false, 10);
            $pdf->Ln(3);
            ...
            $pdf->Cell(1, 7, ':', 0, 0, 'L');
            $pdf->Cell(5);
            $pdf->MultiCellIndent(147, 7, 'Permohonan penerbitan surat tugas dosen penguji Tugas Akhir/Skripsi mahasiswa TA ' . $sgas->tahun_akademik->tahun_akademik . ' ' . $fakultas . ' ITSK RS dr. Soepraoen Malang', 0, 'J', false, 10);
            ...
            $pdf->Cell(3);
            $pdf->Cell(1, 7, 'Nama Dosen Penguji', 0, 0, 'L');
            $pdf->Cell(60);
            $pdf->Cell(1, 7, ':', 0, 0, 'L');
            $pdf->Cell(5);
            ...
            $pdf->Cell(1, 7, ':', 0, 0, 'L');
            $pdf->Cell(5);
            $pdf->MultiCellIndent(147, 7, '1.   Seterimanya Surat ini disamping tugas jabatan dan tanggung jawab sehari-hari ditunjuk sebagai dosen penguji Tugas Akhir/Skripsi mahasiswa Semester ' . $v->semester . ' TA. ' . $sgas->tahun_akademik->tahun_akademik . ' Prodi ' . $sgas->dosen->prodi->nama_prodi . ';', 0, 'J', false, 0);
            $pdf->Ln(3);
            $pdf->Cell(42);
