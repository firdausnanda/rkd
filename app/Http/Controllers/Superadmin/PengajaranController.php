...
class PengajaranController extends Controller
{
    ...
    public function print(Request $request)
    {
        ...
        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell(10, 7,'No', 'TB', 0, 'C');
        $pdf->Cell(25, 7,'Kode MA', 'TB', 0, 'C');
        $pdf->Cell(45, 7,'Matakuliah', 'TB', 0, 'C');
        $pdf->Cell(25, 7,'Prodi', 'TB', 0, 'C');
        $pdf->Cell(25, 7,'Semester', 'TB', 0, 'C');
        $pdf->Cell(20, 7,'Kelas', 'TB', 0, 'C');
        $pdf->Cell(20, 7,'Jumlah Dosen', 'TB', 0, 'C');
        $pdf->Cell(20, 7,'Total', 'TB', 0, 'C');

        $pdf->SetFont('Arial', '', 8);
        foreach($pengajaran as $key => $v){
            $total = $v->total_sks * $v->kelas / $v->total_dosen;
            $pdf->ln();
            $pdf->Cell(10, 7, $key+1, 'TB', 0, 'C');
            $pdf->Cell(25, 7, $v->matakuliah->kode_matakuliah, 'TB', 0, 'L');
            $pdf->Cell(45, 7, $v->matakuliah->nama_matakuliah, 'TB', 0, 'L');
            $pdf->Cell(25, 7, $v->prodi->nama_prodi, 'TB', 0, 'L');
            $pdf->Cell(25, 7, $v->semester, 'TB', 0, 'C');
            $pdf->Cell(20, 7, $v->kelas, 'TB', 0, 'C');
            $pdf->Cell(20, 7, $v->total_dosen, 'TB', 0, 'C');
            $pdf->Cell(20, 7, $total, 'TB', 0, 'C');
        }
